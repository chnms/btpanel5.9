...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh $btsb_Url/install/install-ubuntu.sh && bash install.sh
		exit;
	else
		wget -O install.sh $btsb_Url/install/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
+----------------------------------------------------------------------
| The WebPanel URL will be http://SERVER_IP:58888 when installed.
+----------------------------------------------------------------------

"
echo -e "\033[31m# 注意: 5.x系列Linux面板已于2020年1月1日起终止维护，与技术支持，请考虑安装全新的7.x版本！ 033[0m"
get_node_url(){
...
btsb_Url=https://raw.githubusercontent.com/chnms/btpanel5.9/master
...
wget -O panel.zip $btsb_Url/install/src/panel.zip -T 10
...
echo -e "\033[33mrelease the following port (58888|888|80|443|20|21) in the security group\033[0m"
echo -e "=================================================================="
